...
global.addjarotList = (key, response, isImage, image_url, _db) => {
    var obj_add = {
        key: key,
        response: response,
        isImage: isImage,
...
global.getDatajarotList = (key, _db) => {
    let position = null
...
global.isAlreadyjarotList = (key, _db) => {
    let found = false
    Object.keys(_db).forEach((x) => {
        if (_db[x].key === key) {
            found = true
